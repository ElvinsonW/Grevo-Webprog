...
<body class="p-4 bg-yellow-2">
    ...
    <div class="w-[93vw] ml-[5vw] p-6">
        <div class="flex flex-row justify-between items-center mb-4">
            <h1 class="text-4xl font-bold">Daftar Organisasi</h1>
        </div>
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-green-100 text-left">
                    <tr>
                        ...
                        <th class="px-4 py-3">ID Organisasi</th>
                        <th class="px-4 py-3">Alamat</th>
                        <th class="px-4 py-3">Info Kontak</th>
                ...
                <tbody class="bg-yellow-2">
                        ...
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3">
                                @if(strtolower($org->organization_status) == 'active')
                                    <span class="bg-green-200 text-green-800 px-2 py-1 rounded">Aktif</span>
                                @else
                                    <span class="bg-red-200 text-red-800 px-2 py-1 rounded">Nonaktif</span>
                                @endif
                            </td>
                            <td class="px-4 py-3">
                                <div class="relative group inline-block text-left">
                                    ...
                                    <div class="absolute hidden group-hover:block bg-white border rounded shadow z-10 mt-1 right-0">
                                        <a href="{{ route('admin.organizations.edit', $org->organization_id) }}" class="block px-4 py-2 text-sm hover:bg-gray-100">Edit</a>
                                        <form action="{{ route('admin.organizations.destroy', $org->organization_id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this organization and all its associated trees?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="block w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-100">Hapus</button>
                                        </form>
                                    </div>
                                </div>
                            </td>
                    ...
                    @if ($organizations->isEmpty())
                        <tr>
                            <td colspan="7" class="text-center px-4 py-6 text-gray-500">Tidak ditemukan organisasi.</td>
                        </tr>
                    @endif
                </tbody>
            </table>
