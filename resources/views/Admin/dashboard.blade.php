            ...
            <div class="grid grid-cols-4 gap-6 mb-8">
                @foreach($now as $key => $value)
                    ...
                    <div class="bg-[#DDECD5] p-4 rounded-lg shadow">
                            ...
                            <div>
                                ...
                                <div class="flex flex-row gap-4 items-center">
                                    @if ($key === 'total_sales' | $key === 'net_revenue')
                                        <p class="text-xl font-bold mt-2">Rp {{ number_format($value, 0, ',', '.') }}</p>

                                    @else
                                        <p class="text-xl font-bold mt-2">{{ number_format($value, 0, ',', '.') }}</p>
                                    @endif
                                    @if($status === 'increase')
                                        <img src="{{ asset('images/increase.svg') }}" alt="" class="h-10 w-auto">
                                    @else
                                        <img src="{{ asset('images/decrease.svg') }}" alt="" class="h-10 w-auto">
                                    @endif
                                </div>
                            </div>
                        ...
                        @else
                            <p class="text-xs text-gray-500">{{ abs($growth) }}% {{ $status }} from last month</p>
                        @endif
                    </div>
                @endforeach
            </div>
